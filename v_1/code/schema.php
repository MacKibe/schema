...
class schema extends mutall {
    ...
    //errors are managed by this property
...
class database extends schema {
    ...
    //The user may decide to report the errors in a different way from just 
    ...
    //
    //Check for mutall model consistency, e.g., missing indices, missing primary 
    //keys, invalid data type for primary keys, invalid relations, etc.
    private function check_model_integrity(): bool {
        ...
        foreach ($this->entities as $ename => $entity) {
            ...
            if (!isset($entity->indices)) {
                ...
                //
                //Ensure that the primary key is not used for indexing.
                //
            }
            //
    ...
    private function get_error_report(int &$no_of_errors, string &$report): void {
        ...
        if ($count > 0) {
            //
            $report .= "\n<h2><b><u>There are $count errors in database '$this->name'</u></b></h2>";
            //
            $report .= '<ol>';
    ...
    //Report errors arising out of the activation process, rather than throw 
...
abstract class entity extends schema implements expression {
    ...
    function get_error_report(int &$no_of_errors, string &$report): void {
        ...
        $no_of_errors += $count;
        //
        //Compile the errors if necessary, at the entity level.
        if ($count > 0) {
            //
...
class column extends schema implements expression{
    ...
    public function get_error_report(int &$no_of_errors, string &$report): void {
        ...
        $no_of_errors += $count;
        //
        //Compile the errors if necessary, at the column level.
        if ($count > 0) {
            //
...
class primary extends capture {
    ...
    function verify_integrity() {
        //
        //1. It must be an autonumber 
        if ($this->data_type !== 'int') {//This condition is not enough
            //
...
class attribute extends capture implements expression {
    ...
    //There are no special integrity checks associated with an attribute for 
    //compliance to the mutall_data requirements
    function verify_integrity() {
...
class foreign extends capture implements ilink{
    ...
    //A foreign must have satisfy the following conditions to be compliant with
    //the mutall_data framework
...
class log extends \DOMDocument {
    ...
    function close() {
        ...
        $this->save($_SERVER['DOCUMENT_ROOT'].$this->filename);
    }
