...
class mutall{
    ...
    //2. The arguments are key/value pairs in the frmaat {[key]:value, ...}. In 
    ...
    //Say something about static and object methods......
    ...
    static async fetch($class, $method, args=null, return_value=true){
        ...
        //Get the index file from the session so as to obtain its absolute location 
        //
        //Fetch and wait for the response, using the (shared) index file
        const response = await fetch('export.php', init);
        //
    ...
    //
    //OPens a new window with the given url populates te window with the database and the 
    //the config as window objects then returns the window incase the programmer 
    //wants to add any other additional prperties
    static openwindow(url){
        //
        //create a wondow to open 
        const win = window.location.href=url;
        //
        //Pass the config and the and the database to the new window 
//        win.__dbase__= window.__dbase__;
//        win.__config__= window.__config__;
        //
...
class schema extends mutall{
    ...
    activate_errors(/*static errors[]*/ $static_errors){
        //
        $static_errors.forEach(function(static_error){
           const error= new error(static_error.msg); 
           //
           //offload any additional information eg the additional information
           Object.assign(error, static_error);
           //
...
class entity extends schema{
    ...
    constructor(dbase, static_entity){
        ...
        let this_=this;
        Object.assign(this_, static_entity);
        //
...
//
//
export {database, entity, column, column_attribute, column_primary, column_foreign, mutall};